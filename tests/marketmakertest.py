...
class TestMarketMaker(unittest.TestCase):
    ...
    def testCalcNewPortfolio(self):
        agent = Market_Maker_binary(initial_portfolio=array([3, 7]))
        ...
        agent = Market_Maker_binary(initial_portfolio=array([-1, 2]))
        ...
        agent = Market_Maker_binary(initial_portfolio=array([0, 0]))
        new_portfolio = agent.calculate_new_portfolio_after_trade(1, 1, 2)
        self.assertEqual(new_portfolio[0], 0)
        self.assertEqual(new_portfolio[1], 2)

        agent = Market_Maker_binary(initial_portfolio=array([43, 193]))
        new_portfolio = agent.calculate_new_portfolio_after_trade(1, -1, 2)
